...
class LedgerTransactionTest extends CodecTesting {
  ...
  test("TxCommon No Signers") {
    val res = getOrFailLogging(jsonRoundTrip(json4, Encoder[TxCommon], Decoder[TxCommon]))
    logger.debug(s"Res: $res")
  }

  test("TxCommone") {
    val res = getOrFailLogging(jsonRoundTrip(json, Encoder[TxCommon], Decoder[TxCommon]))
    logger.debug(s"Res: $res")
  }

  test("Failure Case Manual") {
    val res = getOrFailLogging(jsonRoundTrip(json, Encoder[LedgerTransaction], Decoder[LedgerTransaction]))
    logger.debug(s"Res: $res")
  }
  ...
  test("Just the Signers") {
    ...
    val res = getOrFailLogging(jsonRoundTrip(j2, Encoder[Signer], Decoder[Signer]))
  }
  test("All Signers") {
    val j3 =
      """ [
    ...
    val res = getOrFailLogging(jsonRoundTrip(j3, Encoder[Signers], Decoder[Signers]))
  }
}
